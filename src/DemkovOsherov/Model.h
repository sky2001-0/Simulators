#ifndef DEMKOV_OSHEROV_MODEL
#define DEMKOV_OSHEROV_MODEL
...
public:
  ...
  : sim_(sim), norml_(std::sqrt(chirp_rate)), result_dirpath_(result_dirpath)
  {
    index_ = 0;
  ...
  void Save()
  {
    ...
    {
      rs::graph::SetLimitY(g_pure_, {0., 1.01});
      rs::draw::FastSaveToFile(
        g_pure_,
    ...
    for (std::size_t i = 0; i < SizeOfModel; ++i) {
      ...
      rs::draw::DrawLineHorizontal(c, sim_.CalcTheoricalProbability(i), kRed);
